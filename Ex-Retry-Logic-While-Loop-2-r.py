...
def wait_and_retry_until_status_is_success(max_wait= 20, sleep_per_retry= 3):
    ...
    while time.time() < timeout:
        ...
        print("current status is {}".format(is_success))
        
        if is_success:
            print("process is sucess")
            break
        else:
            print("no success sleeping for {} and retrying".format(sleep_per_retry))
            time.sleep(sleep_per_retry)
            continue       
    else:
        raise Exception("didn't get True after trying {} seconds".format(max_wait))
...
def wait_and_retry_until_status_is_success_2(max_wait = 20, sleep_per_retry = 3):
    ...
    while time.time() <= timeout and not is_success:
        ...
        is_success = is_status_success()
        print("status is {}".format(is_success))
        time.sleep(sleep_per_retry)
